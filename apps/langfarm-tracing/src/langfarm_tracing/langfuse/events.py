...
from langfarm_io.langfuse.crud import find_model_by_cache
from langfarm_io.langfuse.schema import Model
from langfarm_io.redis import read_created_at_or_set
...
def read_events_schema_to_dict(schema_name: str) -> dict:
    """
    从 <schema_name>-schema.json 读取 json

    :param schema_name: 相对于 <package_base_dir>/langfuse/events_schema/<schema_name>-schema.json 的文件
    :return: 返回 dict
    """
    script_file = f"/{__name__}.py"
    package_base_dir = __file__[: -len(script_file)]
    with open(f"{package_base_dir}/langfarm_tracing/langfuse/events_schema/{schema_name}-schema.json") as f:
        return json.load(f)
...
class BaseEventHandler:
    def __init__(self, producer: Producer, topic: str):
        self.topic = topic
        self.kafka_sink = KafkaSink(producer, topic)
        self.schema_json = read_events_schema_to_dict(topic)
...
class GenerationHandler(SpanHandler):
    ...
    def _deal_input_usage(self, usage: dict) -> tuple[int, int, int]:
        prompt_tokens = 0
        completion_tokens = 0
        total_tokens = 0
        if "input" in usage:
...
def create_handlers(
    *, kafka_config: KafkaConfig | None = None, kafka_producer: Producer | None = None
) -> dict[str, BaseEventHandler]:
    if kafka_producer is None:
        if kafka_config is None:
            raise ValueError("kafka_producer or kafka_config must be set")
        kafka_producer = get_kafka_producer(kafka_config)

    # create handlers
    trace_handler = TraceHandler(kafka_producer, "traces")
    span_handler = SpanHandler(kafka_producer, "observations")
    generation_handler = GenerationHandler(kafka_producer, "observations")
...
def events_dispose(data: dict, project_id: str, handler_map: dict) -> dict:
    # 取 top metadata
    ...
    for event in batch:
        if "id" in event:
            ...
            if "body" not in event:
                errors.append({"id": event_id, "status": 400, "message": "event 没有找到 body"})
                logger.warning("event=[%s], event 没有找到 body, 此事件丢掉。", event_id)
                continue

            if "timestamp" not in event:
                errors.append({"id": event_id, "status": 400, "message": "event 没有找到 timestamp"})
            ...
            # 没有 id 的 body 丢掉
            if "id" not in body:
                errors.append({"id": event_id, "status": 400, "message": "没有找到 body.id"})
